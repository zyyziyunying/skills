...
Default output is rewritten text in the conversation. Do not write or edit files unless the user explicitly asks for an in-place file edit.
...
2. **Match their voice in the rewrite.** Don't just remove AI patterns - replace them with patterns from the sample. If they write short sentences, don't produce long ones. If they use "stuff" and "things," don't upgrade to "elements" and "components." The no em/en dash rule in §14 overrides punctuation matching: if the sample uses dashes, preserve the cadence with commas, periods, parentheses, or rewritten sentence structure instead.
